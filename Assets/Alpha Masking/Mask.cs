...
namespace ToJ
{
	...
	public class Mask : MonoBehaviour
	{
		...
		private void UpdateUIMaterials (Matrix4x4 worldToMask, Vector4 tilingAndOffset)
        {
#if UNITY_EDITOR
            if (maskeeVersion == 0)
            {
                VersionUpgradeMaskees(maskedImage);
            }
#endif

            if (IsMaskeeMaterial(maskedImage.material))
            {
                UIMaterialModifier modifier = maskedImage.GetComponent<UIMaterialModifier>();
                if (modifier == null)
                {
                    modifier = maskedImage.gameObject.AddComponent<UIMaterialModifier>();
                }

                Canvas currentCanvas = maskedImage.canvas;
                ...
                modifier.SetMaskToMaskee(worldToMask * canvasMatrix, tilingAndOffset, ClampingBorder, IsMaskingEnabled, enableScreenSpaceUI,
                    ClampAlphaHorizontally, ClampAlphaVertically, UseMaskAlphaChannel, false);
		...
		public void UpdateMasking ()
		{
			...
			if (DefaultMaskedSpriteShader == null || DefaultMaskedUnlitShader == null || maskedImage == null)
			{
			...
			if (transform.parent != null)
			{
				...
				if (fullMaskRefresh)
				{
					...
					UpdateUIMaterials(worldToMask, tilingAndOffset);
