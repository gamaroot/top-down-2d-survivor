    ...
    public class SceneContextOperator
    {
        ...
        protected void UpdateScreenContext(SceneContext updatedContext)
        {
            ...
            if (this._screenContext.Exists(x => x.ID == updatedContextRef.ID))
            {
                ...
                if (context.ScreenState != updatedContext.ScreenState)
                    screenStateChangeListener?.Invoke(context.ID, updatedContext.ScreenState);

                if (updatedContextRef.ScreenState != SceneState.NONE)
                    context.ScreenState = updatedContextRef.ScreenState;

                if (updatedContextRef.ScreenState == SceneState.UNLOADED)
                {
            ...
            else
            {
                context = new SceneContext()
                {
                    ...
                    ScreenState = SceneState.NONE

                };
                this._screenContext.Add(updatedContextRef);

                screenStateChangeListener?.Invoke(updatedContext.ID, updatedContext.ScreenState);
            }
        }
